;(function () {
            ...
            ajaxForm_list.each(function(){
            	$(this).validate({
                	...
                	submitHandler:function(form){
                		var $form=$(form);
                        var isDialogFrom = $form.attr('data-is-dialog');
                        var url =  $form.attr('action');
                        if(!$btn){
                            $btn = $('button.js-ajax-submit');
                        }
                        if($btn){
                            if($btn.data('action')){
                                url = $btn.data('action');
    ...
    var monthDayInput = $("input.js-month-day");
    if (monthDayInput.length) {
        Wind.use('datePicker', function () {
            monthDayInput.datePicker({
                startView: 'decade',
                minView: 'decade',
                format: 'mm-dd',
                autoclose: true
    ...
    var yearDayInput = $("input.js-year-day");
    if (yearDayInput.length) {
        Wind.use('datePicker', function () {
            yearDayInput.datePicker({
                startView: 'decade',
                minView: 'decade',
                format: 'yyyy-mm',
                autoclose: true
...
function fadatupian(obj){
    // console.log(obj);
    // Wind.use('imageView',function(){
    //     $(obj).parent().imageView({width:850, height:708})
    //     $(".show_pic_big").imageView({width:850, height:708})
    // });
    // $(obj).css('width','200%');
}
/**
...
function image_preview_dialog(img) {
    ...
    Wind.use("artDialog", function () {
        art.dialog({
            ...
            background: "#CCCCCC",
            opacity: 0,
            content: '<a href="'+img+'" target="_blank"> <img  src="' + img + '" /></a>'
        });
    });
}
 ...
 * @param img 图片地址
 */
function show_image_dialog(img) {
    // return false;
    Wind.use("artDialog", function () {
        art.dialog({
            title: '图片查看',
...
function artdialog_succeed(msg) {
    Wind.use("artDialog", function () {
        art.dialog({
            id: new Date().getTime(),
            icon: "succeed",
...
$("img").error(function(){
    ...
    var isTry = $(this).attr('data-is-try');
    if(isTry==undefined){
        $(this).attr('data-is-try',1);
        // var NewSrc = 'http://carfile.fxeonline.com'+src;
        var host = window.location.host;
        var NewSrc = host+src;
        $(this).attr('src',NewSrc);
        $(this).parent().attr('href',NewSrc);
    }
});
 ...
 * @returns {string}
 */
function show_img_list(id,options){
    id="#"+id;
    var params = {
        photos: id,
        anim: 5
    ...
    Wind.use('layer3', function () {
        index=layer.photos(params);
    });
    return index;
}

function artdialog_desc(content) {
    Wind.use('artDialog', function() {
        //关闭之前的dialog
...
// Wind.use('imageView',function(){
//     $(".show_img").imageView({width:40, height:40})
// });